...
sub run {
    ...
    my($sql,@binds) = $builder->select(undef, [
        ...
        [ \'COUNT(DISTINCT assign.id)'  => 'assign_count' ],
        [ \'COUNT(DISTINCT circle.id)'  => 'circle_count' ],
        [ \'SUM(circle_order.count)'    => 'book_count' ],
        [ \'SUM(circle.circle_point)'   => 'point' ],
        [ \'SUM(circle_book.price * circle_order.count)' => 'book_price' ],
    ], $where, {
        joins => [
            ...
            [ assign_list => { table => 'circle',       condition => 'assign.circle_id = circle.id', type => 'LEFT' }],
            [ assign_list => { table => 'circle_book',  condition => 'circle.id = circle_book.circle_id', type => 'INNER' }],
            [ assign_list => { table => 'circle_order', condition => 'circle_book.id = circle_order.book_id', type => 'INNER' }],
        ],  
